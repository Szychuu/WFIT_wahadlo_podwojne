...
def animate(x1, y1, x2, y2, idx, L1, L2, playback_fps):
    fig, ax = plt.subplots(figsize=(8,6))
    ax.set_xlim(-L1-L2-0.1, L1+L2+0.1)
    ax.set_ylim(-L1-L2-0.1, L1+L2+0.1)
    ax.set_aspect('equal')

    # włączamy siatkę kartezjańską oraz osie x=0 i y=0
    ...
    def update(frame):
        ...
        tr0 = max(0, i - int(1.0/(idx[1]-idx[0] if len(idx)>1 else 1)))
        trail_line.set_data(x2[tr0:i], y2[tr0:i])
        return rod_line, trail_line, bob1, bob2
...
def main():
    theta1_0, theta2_0, L1, L2, m1, m2 = prompt_initial_conditions()
